...
public class UIWrapContent : MonoBehaviour
{
    ...
    Dictionary<int, int> myChildrenPool = new Dictionary<int, int>();
	...
	protected virtual void UpdateItem (Transform item, int index)
	{
		...
		{ 
            ...
            myChildrenPool[index] = realIndex;
			onInitializeItem(item.gameObject, index, realIndex);
		}
	}
    public void UpdateItemByRealIndex(int realindex) 
    {
        foreach (var item in myChildrenPool)
        {
            if (item.Value == realindex) 
            {
                Transform t = mChildren[item.Key];
                if (t != null) 
                {
                    UpdateItem(t, item.Key);
